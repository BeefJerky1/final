...
public class PayController {
	...
	public String pay(
			@ModelAttribute PurchaseVO listVO,
			HttpSession session) throws URISyntaxException {
		
		ClubPlusDto clubPlusDto = clubPlusDao.find(listVO.getClubPlusNo());
		if(clubPlusDto == null) {
		...
		KakaoPayReadyRequestVO requestVO = KakaoPayReadyRequestVO.builder()
																...
																.item_name(clubPlusDto.getClubPlusName())
																.quantity(1)
																.total_amount(clubPlusDto.getClubPlusPrice())
															.build();
		...
		session.setAttribute("purchase", List.of(listVO)); // 실제 구매 상품 명단
...
//		int cancelAmount = paymentDao.calculateCancelAmountByJava(paymentNo);
	...
	// 다중결제 추후에 한번 해보기
//	// purchase[0].no=1&purchase[0].quantity=1 형태로 오는 데이터를 수신하기 위한 처리
//	@PostMapping("/pay2_purchase/{clubPayNo}")
//	public String pay(
//			@PathVariable int clubPayNo,
//			@ModelAttribute PurchaseListVO listVO,
//			HttpSession session) throws URISyntaxException {
//		System.out.println(listVO);
//		System.out.println("되는지 모르겠다");
//		if(listVO == null || listVO.getPurchase() == null) {
//			// 상품을 구매한것이 없으므로 차단
//			return "redirect:member/mypage"; // 또는 에러 처리
//		}
//		
//		System.out.println("되는지 모르겠다2");
