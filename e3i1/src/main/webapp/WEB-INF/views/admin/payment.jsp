							...
							<tbody class="fontSS">
								<template v-for="(payment1 , index) in payment" :key="index">
									<tr class="show-detail">
										...
										<td>
										<div class="dropdown">
  <a class="btn btn-secondary dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" v-on:click="detail(index)" aria-expanded="false">
  	기타
  </a>

  <ul class="dropdown-menu">
    <li><a class="dropdown-item" href="#" data-bs-toggle="modal" data-bs-target="#detailModal">상세보기</a></li>
    <li><a class="dropdown-item" href="#">삭제</a></li>
						...
						<button type="button" v-on:click="append()"
							...
							({{showPayment}}/{{totalPayment}})</button>
					</div>
				</div>
			</div>
			<!-- 오른쪽 사이드바 -->
						...
						<div class="col-lg-12 col-md-12 col-sm-12">
							...
							<form class="form-floating">
								<input type="text" class="form-control" v-model="paymentNo"
									id="paymentNo" placeholder="결제번호"> <label
									for="paymentNo">결제번호</label>
							</form>
							<form class="form-floating">
								<input type="text" class="form-control" v-model="paymentTid"
									id="paymentTid" placeholder="TID"> <label
									for="paymentTid">TID</label>
							</form>
							<form class="form-floating">
								<input type="text" class="form-control" v-model="paymentName"
									id="paymentName" placeholder="상품이름"> <label
									for="paymentName">상품이름</label>
							</form>
							<form class="form-floating">
								<input type="text" class="form-control" v-model="paymentTotal"
									id="paymentTotal" placeholder="금액"> <label
									for="paymentTotal">금액</label>
							</form>
							<form class="form-floating">
								<input type="text" class="form-control" v-model="paymentStatus"
									id="paymentStatus" placeholder="상태"> <label
									for="paymentStatus">상태</label>
							</form>
						</div>
						<div class="col-lg-12 col-md-12 col-sm-12">
							<h5>정렬</h5>
		...
		<div v-if="this.paymentDetail!=null">
			<div class="modal fade" id="detailModal" tabindex="-1"
                ...
                <div class="col-lg-8 col-md-8 col-sm-8 p-4" class="text-start">
		            <h4><b>회원 정보</b></h4>
		            <h5>회원 번호: {{paymentDetail.memberDto.memberNo}}</h5>
		            <h5>회원 닉네임: {{paymentDetail.memberDto.memberNick}}</h5>
                </div>
                <div class="col-lg-8 col-md-8 col-sm-8 p-4">
                	<h4><b>소모임 정보</b></h4>
		            <h5>소모임 번호: {{paymentDetail.clubDto.clubNo}}</h5>
		            <h5>소모임 이름: {{paymentDetail.clubDto.clubName}}</h5>
		            <h5>소모임 대분류: {{paymentDetail.clubDto.clubMainCategory}}</h5>
		            <h5>소모임 소분류: {{paymentDetail.clubDto.clubSubCategory}}</h5>
                </div>
                <div class="col-lg-8 col-md-8 col-sm-8 p-4">
                	<h4><b>결제정보 정보</b></h4>
                	<h5>결제 번호: {{paymentDetail.paymentDto.paymentNo}}</h5>
		            <h5>상품 이름: {{paymentDetail.paymentDto.paymentName}}</h5>
		            <h5>상품 가격: {{paymentDetail.paymentDto.paymentTotal}}원</h5>
		            <h5>결제 상태: {{paymentDetail.paymentDto.paymentStatus}}</h5>
		            </div>
		            <div class="col-lg-12 col-md-12 col-sm-12 p-4 text-end">
...
// 							console.log(i)
		        ...
		        detail(index){
		        	const payment = this.payment[index];
		        	axios({
		        		url:"${pageContext.request.contextPath}/rest/admin/paymentDetail/"+payment.paymentNo,
		        		method:"get",
		        	}).then(resp=>{
